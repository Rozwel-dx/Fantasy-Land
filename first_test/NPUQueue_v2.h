...
private:
  ...
  std::atomic<uint32_t> enqueue_count = 0;
  std::atomic<uint32_t> dequeue_count = 0;
  double last_print_time = 0.0;
  const double print_interval_us = 100.0; // 100 microseconds
  ...
  // Print task queue statistics if the time interval has passed
  void PrintTaskQueueStats();
};
