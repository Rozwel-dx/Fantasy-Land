...
from gil_tracer.file_manager import FileManager
    ...
    def get_real_main_pid_list(self, pid_list: List[Tuple[int, int]], 
        ...
        for pid, ppid in pid_list:
            ...
            for pids in main_pid_list:
                ...
                if real_pid in pids:
                    per_real_pid_list.append(real_pid)
                    continue
                real_ppid = self.get_main_pid_from_docker(ppid)
                if real_ppid in pids:
                    per_real_pid_list.append(real_ppid)
...
def run(args: argparse.Namespace) -> None:
    ...
    if not args.config:
        ...
        if not os.path.exists(args.config):
            logging.error(f"The {args.config} file dose not exist.Please check and try again.")
            return
