...
bool Repository::WriteQueue(void *cur_paras)
{
    ...
    // Calculate queue size before incrementing write index
    uint32_t queue_size = write_idx.idx >= read_idx.idx ? write_idx.idx - read_idx.idx : write_idx.idx + N - read_idx.idx;
    ...
    // Print statistics if time interval has passed
    PrintTaskQueueStats();
    return true;
}
...
bool Repository::ReadQueue()
{
    ...
    // Calculate queue size before reading
    uint32_t queue_size_before = write_idx.idx >= read_idx.idx ? write_idx.idx - read_idx.idx : write_idx.idx + N - read_idx.idx;
...
void Repository::PrintTaskQueueStats() {
    double current_time = GetCurrentTime();
    double elapsed_us = (current_time - last_print_time) * 1000000.0;
    
    if (elapsed_us >= print_interval_us) {
        uint32_t current_enqueue = enqueue_count.load(std::memory_order_relaxed);
        uint32_t current_dequeue = dequeue_count.load(std::memory_order_relaxed);
        
        if (current_enqueue > 0 || current_dequeue > 0) {
            // Calculate current queue size
            uint32_t queue_size = write_idx.idx >= read_idx.idx ? write_idx.idx - read_idx.idx : write_idx.idx + N - read_idx.idx;
            
            logger->info("TaskQueue: Time window [%.6f-%.6f], enqueued = %u, dequeued = %u, current size = %u, device = %d",
                last_print_time, current_time, current_enqueue, current_dequeue, queue_size, device_idx);
