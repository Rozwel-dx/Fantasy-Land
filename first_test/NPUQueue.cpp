...
bool Repository::WriteQueue(void *cur_paras)
{
    ...
    logger->info("TaskQueue: Operator enqueued, current size = %u, time = %lf, device = %d", 
        queue_size + 1, current_time, device_idx);
...
bool Repository::ReadQueue()
{
    ...
    gettimeofday(&tv, NULL);
    double current_time = tv.tv_sec + tv.tv_usec / 1000000.0;
    logger->info("TaskQueue: Operator about to dequeue, current size = %u, time = %lf, device = %d", 
        queue_size_before, current_time, device_idx);
...
bool ReleaseQueue::ReadFromReleaseQueue()
{
    ...
    // Calculate queue size after reading
