...
int GetQueueSize(uint32_t write_idx, uint32_t read_idx) {
    return (write_idx - read_idx + kQueueCapacity) % kQueueCapacity;
}
...
std::string GetCurrentTime() {
    ...
    char buffer[64];
    snprintf(buffer, sizeof(buffer), "%ld.%06ld", tv.tv_sec, tv.tv_usec);
...
bool Repository::WriteQueue(void *cur_paras)
{
    ...
    // Log queue size after enqueue
    int queue_size = GetQueueSize(write_idx.idx, read_idx.idx);
    std::string current_time = GetCurrentTime();
    logger->info("TaskQueue: Operator enqueued, current size = %d, time = %s, device = %d", 
        queue_size, current_time.c_str(), device_idx);
    return true;
}
...
bool Repository::ReadQueue()
{
    ...
    __sync_synchronize();
    // Log queue size before dequeue
    int queue_size_before = GetQueueSize(write_idx.idx, read_idx.idx);
    std::string current_time_before = GetCurrentTime();
    logger->info("TaskQueue: Operator about to dequeue, current size = %d, time = %s, device = %d", 
        queue_size_before, current_time_before.c_str(), device_idx);
    ...
    int queue_size_after = GetQueueSize(write_idx.idx, read_idx.idx);
    std::string current_time_after = GetCurrentTime();
    logger->info("TaskQueue: Operator dequeued, current size = %d, time = %s, device = %d", 
        queue_size_after, current_time_after.c_str(), device_idx);
...
{
    // Log queue size before clearing
    int queue_size_before = GetQueueSize(write_idx.idx, read_idx.idx);
    if (queue_size_before > 0) {
        std::string current_time = GetCurrentTime();
        logger->info("TaskQueue: Queue cleared, size changed from %d to 0, time = %s, device = %d", 
            queue_size_before, current_time.c_str(), device_idx);
    }
